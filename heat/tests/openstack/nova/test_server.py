...
class ServersTest(common.HeatTestCase):
    def setUp(self):
        ...
        self.mock_flavor = mock.Mock(ram=4)
        self.mock_image = mock.Mock(min_ram=1, status='active')
    ...
    def _mock_validate_flavor_image_success(self, image='F17-x86_64-gold',
        ...
        nova.NovaClientPlugin.get_flavor(flavor).AndReturn(
            self.mock_flavor)
    ...
    def test_server_validate_with_bootable_vol(self):
        ...
        flavor = web_server['Properties']['flavor']
        ...
        nova.NovaClientPlugin.get_flavor(
            flavor).MultipleTimes().AndReturn(None)
...
'''
    ...
    def test_validate_insufficient_ram_image(self):
        stack_name = 'test_stack'
        tmpl, stack = self._setup_test_stack(stack_name)

        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_insufficient_ram_image',
                                resource_defns['WebServer'], stack)
