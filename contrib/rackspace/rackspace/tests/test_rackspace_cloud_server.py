...
class CloudServersValidationTests(common.HeatTestCase):
    ...
    def test_validate_no_image(self, mock_client, mock_plugin):
        ...
        mock_plugin().get_image.return_value = None
    ...
    def test_validate_bfv_volume_only(self, mock_client, mock_plugin):
        ...
        mock_image = mock.Mock(status='ACTIVE', min_ram=2)
    ...
    def test_validate_image_flavor_excluded_class(self, mock_client,
        ...
        mock_image = mock.Mock(size=1, status='ACTIVE', min_ram=2)
    ...
    def test_validate_image_not_enough_disk(self, mock_client, mock_plugin):
        server = cloud_server.CloudServer("test", self.rsrcdef, self.mockstack)

        mock_image = mock.Mock(size=1, status='ACTIVE', min_ram=2, min_disk=2)
        mock_image.get.return_value = "standard1"

        mock_flavor = mock.Mock(ram=4, disk=1)
        mock_flavor.to_dict.return_value = {
            'OS-FLV-WITH-EXT-SPECS:extra_specs': {
                'class': 'standard1',
                'disk_io_index': 1,
        ...
        error = self.assertRaises(
            exception.StackValidationFailed, server.validate)
        self.assertEqual(
            'Image CentOS 5.2 requires 2 GB minimum disk space. '
            'Flavor 256 MB Server has only 1 GB.',
            six.text_type(error))

    def test_validate_image_size_zero(self, mock_client, mock_plugin):
        server = cloud_server.CloudServer("test", self.rsrcdef, self.mockstack)

        mock_image = mock.Mock(size=0, status='ACTIVE', min_ram=2, min_disk=2)
        mock_image.get.return_value = "standard1"
        ...
        error = self.assertRaises(
            exception.StackValidationFailed, server.validate)
        self.assertEqual(
            'Image CentOS 5.2 has size <= 0', six.text_type(error))
