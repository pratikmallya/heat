...
class Group(resource.Resource):
    ...
    def _get_launch_config_args(self, launchconf):
        ...
        image_id = server_args[self.LAUNCH_CONFIG_ARGS_SERVER_IMAGE_REF]
        if image_id:
            image_id = self.client_plugin('glance').get_image_id(image_id)
        flavor_id = server_args[self.LAUNCH_CONFIG_ARGS_SERVER_FLAVOR_REF]
        if flavor_id:
            flavor_id = self.client_plugin('nova').get_flavor_id(flavor_id)
        return dict(
